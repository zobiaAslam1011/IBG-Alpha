...
add_action( 'pre_get_posts', function ( $q )
{
    if (  !is_admin() // Only target front end queries
        ...
        $q->set( 'post_type', ['post', 'blog_post','trenging','services','portfolio','our_features','partners','our_team'] ); 
    }
});
...
function namespace_add_custom_types( $query ) {
  if( (is_category() || is_tag()) && $query->is_archive() && empty( $query->query_vars['suppress_filters'] ) ) {
    $query->set( 'post_type', array(
     'post', 'blog_post','trenging','services','portfolio','our_features','partners','our_team'
        ));
    }
}
...
function hidenotice_admin_theme_style() {
   echo '<style>.update-message.notice.inline.notice-warning.notice-alt, #menu-posts, #menu-plugins span.update-plugins.count-6 { display: none !important; }</style>';
}
